...
// infinite pagination of the gallery part of the site
    ...
    if (isset($_SESSION["loggedin"]) && ($_POST['like'] || $_POST['unlike'])){
        ...
        if ($_POST['unlike']){
            ...
            $r = $db->prepare($sql);
            $r->execute([ $pid ]);
            $sql = "DELETE FROM likes WHERE userid = '$userid' AND photoid = ?";
            $rE = $db->prepare($sql);
            $rE->execute([ $pid ]);
        }
...
<body>
    ...
    <div class="posts">
        <?php
            ...
            $sql = "SELECT id, username, photo FROM users_imgs WHERE id = ?";
            $qr = $db->prepare($sql);
            $qr->execute([ $imgid ]);
            while ($a = $qr->fetch()) {
                ?>
            ...
            ?>
                ...
                while ($x = $q->fetch()) {
                    ...
                    $cmt = $x["comment"];
                    $userc = $x["username"];
                    echo "<div class=\"com\"><b>$userc:</b> $cmt<br></div>";
                }
                ?>
                </div>
                <?php 
        ...
        //if photo does not exists
        if (!isset($photo)){
            ?>
            <div class="error">this picture is ether is deleted by owner or does not exists</div>
            <?php
        } 
        ?>
    </div>
